...
$query_Recgallery = "SELECT * FROM data_set WHERE d_class1 = '$menu_is' ORDER BY d_sort ASC, d_date DESC";
$query_limit_Recgallery = sprintf("%s LIMIT %d, %d", $query_Recgallery, $startRow_Recgallery, $maxRows_Recgallery);
$Recgallery = $conn->query($query_limit_Recgallery);
$row_Recgallery = $Recgallery->fetch();

if (isset($_GET['totalRows'])) {
    $totalRows = $_GET['totalRows'];
} else {
    $all_Recgallery = $conn->query($query_Recgallery);
    $totalRows = $all_Recgallery->rowCount();
}
...
if ($G_changeSort == 1 || $G_delchangeSort == 1) {
    ...
    $Recgallery = $conn->query($query_Recgallery);
    $row_Recgallery = $Recgallery->fetch();

    do {
        if ($row_Recgallery['d_sort'] == 0) {} else if ($row_Recgallery['d_id'] == $_GET['now_d_id']) {
            ...
            $stat->bindParam(':d_id', $row_Recgallery['d_id'], PDO::PARAM_INT);
            $stat->execute();
    ...
    } while ($row_Recgallery = $Recgallery->fetch());
    ...
    if ($G_changeSort == 1) {
        header("Location:gallery_list.php?pageNum=" . $_GET['pageNum'] . "&totalRows=" . $_GET['totalRows']);
    } else if ($G_delchangeSort == 1) {
        header("Location:gallery_list.php?pageNum=" . $_GET['pageNum']);
    }
}
                            ...
                            <form action="gallery_process.php" method="post" name="form1" id="form1">
                                ...
                                <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1">
                                    ...
                                    do {
                                        ...
                                        $query_RecImage = "SELECT * FROM file_set WHERE file_type='galleryCover' AND file_d_id = :file_d_id";
                                    ...
                                    <tr <?php if ($i%2==0): ?>bgcolor='#E4E4E4'<?php endif ?>>
                                        <td align="center" class="table_data">
                                            <a href="gallery_edit.php?d_id=<?php echo $row_Recgallery['d_id']; ?>">
                                                <?php echo $row_Recgallery['d_date']; ?>
                                            </a>
                                        </td>
                                        <td align="center" class="table_data">
                                            <select name="d_sort" id="d_sort" onchange="changeSort('<?php echo $pageNum; ?>','<?php echo $totalRows; ?>','<?php echo $row_Recgallery['d_id']; ?>',this.options[this.selectedIndex].value)">
                                                <option value="0" <?php if (!(strcmp(0, $row_Recgallery[ 'd_sort']))) {echo "selected"; } ?>>至頂</option>
                                                <?php
                                        ...
                                        <td align="center" class="table_data">
                                            <a href="gallery_edit.php?d_id=<?php echo $row_Recgallery['d_id']; ?>">
                                                <?php echo $row_Recgallery['d_title']; ?>
                                            </a>
                                        </td>
                                        <td align="center" class="table_data"><a href="gallery_edit.php?d_id=<?php echo $row_Recgallery['d_id']; ?>">
                                        ...
                                        <!-- <td align="center" class="table_data">
                                            <?php  //list使用
                                            if($row_Recgallery['d_level']) {
                                                echo "<a href='".$row_Recgallery['d_level']."' rel='".$row_Recgallery['d_id']."' class='activeChL'><img src=\"image/accept.png\" width=\"16\" height=\"16\"  ></a>";
                                            } else {
                                                echo "<a href='".$row_Recgallery['d_level']."' rel='".$row_Recgallery['d_id']."' class='activeChL'><img src=\"image/delete.png\" width=\"16\" height=\"16\"  ></a>";
                                            }
                                            ?>
                                        </td> -->
                                        <td align="center" class="table_data">
                                    ...
                                    <?php } while ($row_Recgallery = $Recgallery->fetch()); ?>
                                </table>
                                <?php } // Show if recordset not empty ?>
                            </form>
                            <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#E1E1E1" class="list_title_table">
...
<script type="text/javascript">
    function changeSort(pageNum, totalRows, now_d_id, change_num) { //v1.0
        ...
        window.location.href = "gallery_list.php?pageNum=" + pageNum + "&totalRows=" + totalRows + "&changeSort=1" + "&now_d_id=" + now_d_id + "&change_num=" + change_num;
    }
</script>