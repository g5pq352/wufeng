...
if (isset($_SESSION['selected_leaderC'])) {
    $G_selected1 = $_SESSION['selected_leaderC'];
}
...
$query_RecleaderY = "SELECT * FROM class_set WHERE c_parent = 'leaderY' AND c_active='1' ORDER BY c_sort ASC, c_id DESC";
$RecleaderY = $conn->query($query_RecleaderY);
$row_RecleaderY = $RecleaderY->fetch();
$totalRows_RecleaderY = $RecleaderY->rowCount();
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <select name="d_class2" id="d_class2" class="chosen-select">
                                                            <?php do { ?>
                                                            <option value="<?php echo $row_RecleaderC['c_id']?>" <?php if (!(strcmp($row_RecleaderC[ 'c_id'], $G_selected1))) {echo "selected";} ?>>
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
    ...
    $stat->bindParam(':d_class1', $class1 = 'leader', PDO::PARAM_STR);
    ...
    if ($ifFile) {
        $file_result = file_process($conn, "leaderFile", "add");
    ...
    $insertGoTo = "leader_list.php?selected1=" . $_POST['d_class2'] . "&pageNum=0&totalRows=" . ($_SESSION['totalRows'] + 1) . "&changeSort=1&now_d_id=" . $new_data_num . "&change_num=1";
