                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                        <td align="left">
                            ...
                            <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                <tr>
                                    <td width="300" class="list_title">刪除客戶</td>
                                    <td width="724">&nbsp;</td>
                                </tr>
                            </table>
                            <table width="100%" border="0" cellspacing="0" cellpadding="0">
...
if ((isset($_REQUEST['user_id'])) && ($_REQUEST['user_id'] != "") && (isset($_REQUEST['delsure']))) {
    ...
    $deleteSQL = "DELETE FROM client WHERE user_id=:user_id";

    $sth = $conn->prepare($deleteSQL);
    $sth->bindParam(':user_id', $_REQUEST['user_id'], PDO::PARAM_INT);
    $sth->execute();

    $deleteGoTo = "client_list.php?delchangeSort=1";
    if (isset($_SERVER['QUERY_STRING'])) {
        $deleteGoTo .= (strpos($deleteGoTo, '?')) ? "&" : "?";
        $deleteGoTo .= $_SERVER['QUERY_STRING'] . "&pageNum_Recclient=" . $_SESSION["ToPage"];
    }
