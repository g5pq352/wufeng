...
$RecnewsC = $conn->query($query_limit_RecnewsC);
$row_RecnewsC = $RecnewsC->fetch();

if (isset($_GET['totalRows_RecnewsC'])) {
    $totalRows_RecnewsC = $_GET['totalRows_RecnewsC'];
} else {
    $all_RecnewsC = $conn->query($query_RecnewsC);
    $totalRows_RecnewsC = $all_RecnewsC->rowCount();
}
...
if ($G_changeSort == 1 || $G_delchangeSort == 1) {
    ...
    do {
        if ($row_RecnewsC['c_sort'] == 0) {} else if ($row_RecnewsC['c_id'] == $_GET['now_c_id']) {
            // echo $sort_num . "<br/>";
            ...
            $stat->bindParam(':c_id', $row_RecnewsC['c_id'], PDO::PARAM_INT);
            $stat->execute();
                        ...
                        <td align="left">
                            ...
                            <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#E1E1E1" class="list_title_table">
                                <tr>
                                    <td width="739" align="right" class="page_display">
                                        <!-------顯示頁選擇與分頁設定開始---------->
                                        <?php displayPages($pageNum, $queryString_RecnewsC, $totalPages_RecnewsC, $totalRows_RecnewsC, $currentPage); ?>
                                        <!-------顯示頁選擇與分頁設定結束---------->
                                    </td>
                                    <td width="110" align="right" class="page_display">
                                        <?php if ($totalRows_RecnewsC > 0) { // Show if recordset not empty ?> 頁數:
                                        <?php echo (($pageNum+1)."/".($totalPages_RecnewsC+1)); ?>
                                        <?php } // Show if recordset not empty ?>
                                    </td>
                                    <td width="151" align="right" class="page_display">所有資料數:
                                        <?php echo $totalRows_RecnewsC ?> </td>
                            ...
                            <form action="newsC_process.php" method="post" name="form1" id="form1">
                                ...
                                <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1">
                                    ...
                                    <tr <?php if ($i%2==0): ?>bgcolor='#E4E4E4'<?php endif ?>>
                                        <td align="center" class="table_data">
                                            <select name="c_sort" id="c_sort" onchange="changeSort('<?php echo $pageNum; ?>','<?php echo $totalRows_RecnewsC; ?>','<?php echo $row_RecnewsC['c_id']; ?>',this.options[this.selectedIndex].value)">
                                                <option value="0" <?php if (!(strcmp(0, $row_RecnewsC[ 'c_sort']))) {echo "selected";} ?>>至頂</option>
                                                <?php
                                            ...
                                            <?php $_SESSION['totalRows']=$totalRows_RecnewsC; ?>
                                        </td>
                                        <td align="center" class="table_data">
                                            <a href="newsC_edit.php?c_id=<?php echo $row_RecnewsC['c_id']; ?>">
                                    ...
                                    <?php } while ($row_RecnewsC = $RecnewsC->fetch()); ?>
                                </table>
                                <?php } // Show if recordset not empty ?>
                            </form>
                            <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#E1E1E1" class="list_title_table">
...
<script type="text/javascript">
    function changeSort(pageNum, totalRows_RecnewsC, now_c_id, change_num) {
