...
$query_RecshopC_level1 = "SELECT * FROM class_set WHERE c_parent = 'shopC' AND c_active='1' AND c_level='1' ORDER BY c_sort ASC, c_id DESC";
$RecshopC_level1 = $conn->prepare($query_RecshopC_level1);
$RecshopC_level1->execute();
$row_RecshopC_level1 = $RecshopC_level1->fetch();
$totalRows_RecshopC_level1 = $RecshopC_level1->rowCount();

$G_selected1 = '';
if (isset($_SESSION['selected_shopC'])) {
    $G_selected1 = $_SESSION['selected_shopC'] = $row_RecshopC['c_link'];
}
                                                                        ...
                                                                        <tr>
                                                                            ...
                                                                            <td>
                                                                                <select name="c_link" id="c_link" class="chosen-select">
                                                                                    <?php do { ?>
                                                                                    <option value="<?php echo $row_RecshopC_level1['c_id']?>" <?php if (!(strcmp($row_RecshopC_level1[ 'c_id'], $G_selected1))) {echo "selected";} ?>>
                                                                                        <?php echo $row_RecshopC_level1['c_title']?>
                                                                                    </option>
                                                                                    <?php
                                                                                    } while ($row_RecshopC_level1 = $RecshopC_level1->fetch());
                                                                                    $rows = $RecshopC_level1->rowCount();
                                                                                    if($rows > 0) {
                                                                                        $RecshopC_level1->execute();
                                                                                    }
                                                                                    ?>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
    ...
    $updateGoTo = "shopC_level2_list.php?selected1=" . $_POST['c_link'] . "&change_num=1&now_c_id=" . $_POST['c_id'] . "&totalRows_RecshopC=" . $_SESSION['totalRows'] . "&pageNum=" . $_SESSION["ToPage"];
