...
$query_RecshopC = "SELECT * FROM class_set WHERE c_parent = 'shopC' AND c_active='1' ORDER BY c_sort ASC, c_id DESC";
                                            ...
                                            <table width="100%" border="0" cellpadding="5" cellspacing="3">
                                                <tr>
                                                    ...
                                                    <td class="table_data">
                                                        ...
                                                        <?php
                                                        do {
                                                            if (!(strcmp($row_RecshopC['c_id'], $row_Recshop['d_class3']))) {
                                                            echo $row_RecshopC['c_title'];
                                                            }
                                                        ...
                                                        <?php
                                                        do {
                                                            if (!(strcmp($row_RecshopC['c_id'], $row_Recshop['d_class2']))) {
                                                            echo $row_RecshopC['c_title'];
                                                            }
                                                        } while ($row_RecshopC = $RecshopC->fetch());
                                                        $rows = $RecshopC->rowCount();
...
if ((isset($_REQUEST['d_id'])) && ($_REQUEST['d_id'] != "") && (isset($_REQUEST['delsure']))) {
    ...
    //----------刪除連動資料到資料庫begin(在主資料前)-----
    $deleteSQL = "DELETE FROM tab_set WHERE tab_type='shop' AND tab_d_id=:tab_d_id";

    $sth = $conn->prepare($deleteSQL);
    $sth->bindParam(':tab_d_id', $_POST['d_id'], PDO::PARAM_INT);
    $sth->execute();
    //----------刪除連動資料到資料庫end(在主資料前)-----
    ...
    $deleteGoTo = "shop_list.php?delchangeSort=1&selected1=" . $G_selected1 . "&selected2=" . $G_selected2;
