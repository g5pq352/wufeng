...
$totalRowsC = $RecaboutC->rowCount();

$G_selected1 = '';
if (isset($_GET['selected1'])) {
    $_SESSION['selected_aboutC'] = $G_selected1 = $_GET['selected1'];
} else {
    $G_selected1 = $_SESSION['selected_aboutC'] = $row_RecaboutC['c_id'];
}

$query_Recabout = "SELECT data_set.*, class_set.c_title as c_title FROM data_set LEFT JOIN class_set ON data_set.d_class2 =class_set.c_id WHERE d_class1 = 'about' AND d_class2='" . $G_selected1 . "' ORDER BY d_sort ASC, d_date DESC";
$query_limit_Recabout = sprintf("%s LIMIT %d, %d", $query_Recabout, $startRow_Recabout, $maxRows_Recabout);
$Recabout = $conn->query($query_limit_Recabout);
$row_Recabout = $Recabout->fetch();
//$_SESSION['selected_about']=$G_selected2;
//echo $query_Recabout;
...
$all_Recabout = $conn->query($query_Recabout);
$totalRows = $all_Recabout->rowCount();

$all_Recabout = $conn->query($query_Recabout);
$totalRows = $all_Recabout->rowCount();
$totalPages_Recabout = ceil($totalRows / $maxRows_Recabout) - 1;
...
$queryString_Recabout = sprintf("&totalRows=%d%s", $totalRows, $queryString_Recabout);
...
else if ($R_pageNum > $totalPages_Recabout) {
...
if ($R_pageNum > $totalPages_Recabout && $R_pageNum != 0) {
    header("Location:about_list.php?pageNum=" . $totalPages_Recabout);
}
...
if ($G_changeSort == 1 || $G_delchangeSort == 1) {
    ...
    $query_Recabout = "SELECT data_set.*, class_set.c_title as c_title FROM data_set LEFT JOIN class_set ON data_set.d_class2 =class_set.c_id WHERE d_class1 = 'about' AND d_class2='" . $G_selected1 . "' ORDER BY d_sort ASC, d_date DESC";
    $_SESSION['selected_aboutC'] = $G_selected1;

    $Recabout = $conn->query($query_Recabout);
    $row_Recabout = $Recabout->fetch();

    do {
        if ($row_Recabout['d_sort'] == 0) {} else if ($row_Recabout['d_id'] == $_GET['now_d_id']) {
            ...
            $stat->bindParam(':d_id', $row_Recabout['d_id'], PDO::PARAM_INT);
            $stat->execute();
    ...
    } while ($row_Recabout = $Recabout->fetch());
    ...
    if ($G_changeSort == 1) {
        if (isset($_GET['now_d_id'])) {
            header("Location:about_list.php?selected1=" . $G_selected1 . "&pageNum=" . $pageNum . "&totalRows=" . $_GET['totalRows'] . "#" . $_GET['now_d_id']);
        } else {
            header("Location:about_list.php?selected1=" . $G_selected1 . "&pageNum=" . $pageNum . "&totalRows=" . $_GET['totalRows']);
        }
    } else if ($G_delchangeSort == 1) {
        header("Location:about_list.php?selected1=" . $G_selected1 . "&pageNum=" . $pageNum);
    }
}
                                ...
                                <tr>
                                    ...
                                    <td width="874"><span class="table_data">分類：
                                        <select name="select1" id="select1" class="chosen-select">
                                            <?php do {?>
                                                <option value="<?php echo $row_RecaboutC['c_id']?>"<?php if (!(strcmp($row_RecaboutC['c_id'], $G_selected1))) {echo "selected=\"selected\"";} ?>><?php echo $row_RecaboutC['c_title']?><?php //echo $row_RecaboutC['c_id']?></option>
                                                <?php
                            ...
                            <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#E1E1E1" class="list_title_table">
                                <tr>
                                    <td width="739" align="right" class="page_display">
                                        <!-------顯示頁選擇與分頁設定開始---------->
                                        <?php displayPages($pageNum, $queryString_Recabout, $totalPages_Recabout, $totalRows, $currentPage); ?>
                                        <!-------顯示頁選擇與分頁設定結束---------->
                                    </td>
                                    <td width="110" align="right" class="page_display">
                                        <?php if ($totalRows > 0) { // Show if recordset not empty ?> 頁數:
                                        <?php echo (($pageNum+1)."/".($totalPages_Recabout+1)); ?>
                            ...
                            <form action="about_process.php" method="post" name="form1" id="form1">
                                ...
                                <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1">
                                    ...
                                    do {
                                        ...
                                        $query_RecImage = "SELECT * FROM file_set  WHERE file_type='aboutCover' AND file_d_id = ".$row_Recabout['d_id'];
                                    ...
                                    <tr <?php if ($i%2==0): ?>bgcolor='#E4E4E4'<?php endif ?>>
                                        ...
                                        <td align="center" class="table_data">
                                            <select name="d_sort" id="d_sort" onchange="changeSort('<?php echo $pageNum; ?>','<?php echo $totalRows; ?>','<?php echo $row_Recabout['d_id']; ?>',this.options[this.selectedIndex].value,<?= $G_selected1 ?>)">
                                                <option value="0" <?php if (!(strcmp(0, $row_Recabout[ 'd_sort']))) {echo "selected";} ?>>至頂</option>
                                                <?php
                                        ...
                                        <td align="center" class="table_data">
                                            <a href="about_edit.php?d_id=<?php echo $row_Recabout['d_id']; ?>">
                                                <?php echo $row_Recabout['d_title']; ?>
                                            </a>
                                        </td>
                                        <td align="center" class="table_data"><a href="about_edit.php?d_id=<?php echo $row_Recabout['d_id']; ?>">
                                    ...
                                    <?php } while ($row_Recabout = $Recabout->fetch()); ?>
                                </table>
                                <?php } // Show if recordset not empty ?>
                            </form>
                            <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#E1E1E1" class="list_title_table">
...
<script type="text/javascript">
	...
	function changeSort(pageNum, totalRows, now_d_id, change_num, selected1) {
	    window.location.href = "about_list.php?selected1=" + selected1 + "&changeSort=1" + "&now_d_id=" + now_d_id + "&change_num=" + change_num + "&pageNum=" + pageNum + "&totalRows=" + totalRows;
	}

	$(document).ready(function() {
		$('#select1').change(function() {
			window.location.href = "about_list.php?changeSort=1&selected1="+$(this).val();
		});
	});
</script>