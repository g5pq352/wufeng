...
if (isset($_REQUEST['type']) && $_REQUEST['type'] == 'newsCover') {
    ...
    $not = $imagesSize['neighborhoodsliderCover']['note'];
    $IWidth = $imagesSize['neighborhoodsliderCover']['IW'];
    $IHeight = $imagesSize['neighborhoodsliderCover']['IH'];
} elseif (isset($_REQUEST['type']) && $_REQUEST['type'] == 'indexsliderCover') {
    $type = $_REQUEST['type'];
    $fileType = "file_type='indexsliderCover' AND";
    $not = $imagesSize['indexsliderCover']['note'];
    $IWidth = $imagesSize['indexsliderCover']['IW'];
    $IHeight = $imagesSize['indexsliderCover']['IH'];
} elseif (isset($_REQUEST['type']) && $_REQUEST['type'] == 'indexprojectsCover') {
    $type = $_REQUEST['type'];
    $fileType = "file_type='indexprojectsCover' AND";
    $not = $imagesSize['indexprojectsCover']['note'];
    $IWidth = $imagesSize['indexprojectsCover']['IW'];
    $IHeight = $imagesSize['indexprojectsCover']['IH'];
} elseif (isset($_REQUEST['type']) && $_REQUEST['type'] == 'freshCover') {
    $type = $_REQUEST['type'];
    $fileType = "file_type='freshCover' AND";
    $not = $imagesSize['freshCover']['note'];
    $IWidth = $imagesSize['freshCover']['IW'];
    $IHeight = $imagesSize['freshCover']['IH'];
} elseif (isset($_REQUEST['type']) && $_REQUEST['type'] == 'indexprojectsCover') {
    $type = $_REQUEST['type'];
    $fileType = "file_type='indexprojectsCover' AND";
    $not = $imagesSize['indexprojectsCover']['note'];
    $IWidth = $imagesSize['indexprojectsCover']['IW'];
    $IHeight = $imagesSize['indexprojectsCover']['IH'];
} elseif (isset($_REQUEST['type']) && $_REQUEST['type'] == 'projectCover') {
    $type = $_REQUEST['type'];
    $fileType = "file_type='projectCover' AND";
    $not = $imagesSize['projectCover']['note'];
    $IWidth = $imagesSize['projectCover']['IW'];
    $IHeight = $imagesSize['projectCover']['IH'];
} elseif (isset($_REQUEST['type']) && $_REQUEST['type'] == 'lifeCover') {
    $type = $_REQUEST['type'];
    $fileType = "file_type='lifeCover' AND";
    $not = $imagesSize['lifeCover']['note'];
    $IWidth = $imagesSize['lifeCover']['IW'];
    $IHeight = $imagesSize['lifeCover']['IH'];
} elseif (isset($_REQUEST['type']) && $_REQUEST['type'] == 'lifeSide') {
    $type = $_REQUEST['type'];
    $fileType = "file_type='lifeSide' AND";
    $not = $imagesSize['lifeSide']['note'];
    $IWidth = $imagesSize['lifeSide']['IW'];
    $IHeight = $imagesSize['lifeSide']['IH'];
} else {
    $type = '-1';
    $fileType = "file_type='image' AND";
    $not = $imagesSize[$_SESSION['nowMenu']]['note'];
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
    ...
    $image_result_check = image_process($conn, $_FILES['image'], $_REQUEST['file_title'], $_SESSION['nowMenu'], "edit", $IWidth, $IHeight);
                ...
                <td>
                    <table width="100%" border="0" cellspacing="3" cellpadding="5">
                        <tr>
                            ...
                            <?php if($_SESSION['nowMenu']=='collection' || $_SESSION['nowMenu']=='progress'){ ?>
                            <tr>
