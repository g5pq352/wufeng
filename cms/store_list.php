...
$query_Recstore = "SELECT * FROM data_set WHERE d_class1 = '$menu_is' ORDER BY d_sort ASC, d_date DESC";
...
$totalPages = ceil($totalRows / $maxRows_Recstore) - 1;
...
if ($R_pageNum > $totalPages && $R_pageNum != 0) {
    header("Location:store_list.php?pageNum=" . $totalPages);
}
...
if ($G_changeSort == 1 || $G_delchangeSort == 1) {
    ...
    $query_Recstore = "SELECT * FROM data_set WHERE d_class1 = 'store' ORDER BY d_sort ASC, d_date DESC";
    ...
    if ($G_changeSort == 1) {
        header("Location:store_list.php?pageNum=" . $_GET['pageNum'] . "&totalRows=" . $_GET['totalRows']);
    } else if ($G_delchangeSort == 1) {
        header("Location:store_list.php?pageNum=" . $_GET['pageNum']);
    }
}
                            ...
                            <form action="store_process.php" method="post" name="form1" id="form1">
                                ...
                                <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1">
                                    ...
                                    do {
                                        ...
                                        $query_RecImage = "SELECT * FROM file_set WHERE file_type='storeCover' AND file_d_id = :file_d_id";
                                    ...
                                    <tr <?php if ($i%2==0): ?>bgcolor='#E4E4E4'<?php endif ?>>
                                        ...
                                        <td align="center" class="table_data">
                                            <select name="d_sort" id="d_sort" onchange="changeSort('<?php echo $pageNum; ?>','<?php echo $totalRows; ?>','<?php echo $row_Recstore['d_id']; ?>',this.options[this.selectedIndex].value)">
                                        ...
                                        <!-- <td align="center" class="table_data">
                                            <?php  //list使用
                                            if($row_Recstore['d_level']) {
                                                echo "<a href='".$row_Recstore['d_level']."' rel='".$row_Recstore['d_id']."' class='activeChL'><img src=\"image/accept.png\" width=\"16\" height=\"16\"  ></a>";
                                            } else {
                                                echo "<a href='".$row_Recstore['d_level']."' rel='".$row_Recstore['d_id']."' class='activeChL'><img src=\"image/delete.png\" width=\"16\" height=\"16\"  ></a>";
                                            }
                                            ?>
                                        </td> -->
                                        <td align="center" class="table_data">
...
<script type="text/javascript">
    function changeSort(pageNum, totalRows, now_d_id, change_num) { //v1.0
        ...
        window.location.href = "store_list.php?pageNum=" + pageNum + "&totalRows=" + totalRows + "&changeSort=1" + "&now_d_id=" + now_d_id + "&change_num=" + change_num;
    }
</script>