...
$maxRows_Reccontactus = 20;
$pageNum = 0;
if (isset($_GET['pageNum'])) {
    $pageNum = $_GET['pageNum'];
}
$startRow_Reccontactus = $pageNum * $maxRows_Reccontactus;

$query_Reccontactus = "SELECT * FROM data_set WHERE d_class1 = '$menu_is' ORDER BY d_date DESC";
$query_limit_Reccontactus = sprintf("%s LIMIT %d, %d", $query_Reccontactus, $startRow_Reccontactus, $maxRows_Reccontactus);
$Reccontactus = $conn->query($query_limit_Reccontactus);
$row_Reccontactus = $Reccontactus->fetch();

if (isset($_GET['totalRows'])) {
    $totalRows = $_GET['totalRows'];
} else {
    $all_Reccontactus = $conn->query($query_Reccontactus);
    $totalRows = $all_Reccontactus->rowCount();
}
$totalPages = ceil($totalRows / $maxRows_Reccontactus) - 1;
...
if ($G_changeSort == 1 || $G_delchangeSort == 1) {
    ...
    $query_Reccontactus = "SELECT * FROM data_set WHERE d_class1 = 'contactus' ORDER BY d_date DESC";
    $Reccontactus = $conn->query($query_Reccontactus);
    $row_Reccontactus = $Reccontactus->fetch();

    do {
        if ($row_Reccontactus['d_sort'] == 0) {} else if ($row_Reccontactus['d_id'] == $_GET['now_d_id']) {
            ...
            $stat->bindParam(':d_id', $row_Reccontactus['d_id'], PDO::PARAM_INT);
            $stat->execute();
    ...
    } while ($row_Reccontactus = $Reccontactus->fetch());
                            ...
                            <form action="contactus_process.php" method="post" name="form1" id="form1">
                                ...
                                <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1">
                                    ...
                                    do {
                                        $i++;
                                        $colname_RecImage = "-1";
                                        if (isset($row_Reccontactus['d_id'])) {
                                          $colname_RecImage = $row_Reccontactus['d_id'];
                                        }
                                        $query_RecImage = "SELECT * FROM file_set WHERE file_type='contactusCover' AND file_d_id = :file_d_id";
                                    ...
                                    <tr <?php if ($i%2==0): ?>bgcolor='#E4E4E4'<?php endif ?>>
                                        <td align="center" class="table_data">
                                            <a href="contact_edit.php?d_id=<?php echo $row_Reccontactus['d_id']; ?>">
                                                <?php echo $row_Reccontactus['d_date']; ?>
                                            </a>
                                        </td>
                                        <td align="center" class="table_data">
                                            <a href="contact_edit.php?d_id=<?php echo $row_Reccontactus['d_id']; ?>">
                                                <?php echo $row_Reccontactus['d_title']; ?>
                                            </a>
                                        </td>
                                        <td align="center" class="table_data"><a href="contact_edit.php?d_id=<?php echo $row_Reccontactus['d_id']; ?>"><img src="image/pencil.png" width="16" height="16" /></a></td>
                                        <td align="center" class="table_data"><a href="contact_del.php?d_id=<?php echo $row_Reccontactus['d_id']; ?>"><img src="image/cross.png" width="16" height="16" /></a></td>
                                    </tr>
                                    <?php } while ($row_Reccontactus = $Reccontactus->fetch()); ?>
                                </table>
                                <?php } // Show if recordset not empty ?>
                            </form>
                            <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#E1E1E1" class="list_title_table">
