...
<?php
if ((isset($_REQUEST['l_id'])) && ($_REQUEST['l_id'] != "") && (isset($_REQUEST['delsure']))) {

    $deleteSQL = "DELETE FROM latest_set WHERE l_id=:l_id";

    $sth = $conn->prepare($deleteSQL);
    $sth->bindParam(':l_id', $_REQUEST['l_id'], PDO::PARAM_INT);
    $sth->execute();

    $deleteGoTo = "latestC_list.php?delchangeSort=1";
