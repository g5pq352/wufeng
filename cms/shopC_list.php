...
if (isset($_GET['selected1'])) {
    ...
    $G_selected1_SQL = "AND c_class=" . $_GET['selected1'];
} else if ($row_ReccityC['c_id']) {
    $_SESSION['selected_shopC'] = $G_selected1 = $row_ReccityC['c_id'];
    $G_selected1_SQL = "AND c_class=" . $row_ReccityC['c_id'];
} else{
    $G_selected1_SQL = "AND c_class=-1";
}



$query_RecshopC = "SELECT * FROM class_set WHERE c_parent = 'shopC' AND c_level='1' $G_selected1_SQL ORDER BY c_sort ASC, c_id DESC";
...
if ($R_pageNum > $totalPages_RecshopC && $R_pageNum != 0) {
    header("Location:shopC_list.php?pageNum=" . $totalPages_RecshopC);
}
...
if ($G_changeSort == 1 || $G_delchangeSort == 1) {
    ...
    if ($G_changeSort == 1) {
        if (isset($_GET['now_c_id'])) {
            header("Location:shopC_list.php?selected1=" . $G_selected1 . "&pageNum=" . $_GET['pageNum'] . "&totalRows=" . $_GET['totalRows_RecshopC'] . "#" . $_GET['now_c_id']);
        } else {
            header("Location:shopC_list.php?selected1=" . $G_selected1 . "&pageNum=" . $_GET['pageNum'] . "&totalRows=" . $_GET['totalRows_RecshopC']);
        }
    } else if ($G_delchangeSort == 1) {
        header("Location:shopC_list.php?selected1=" . $G_selected1 . "&pageNum=" . $_GET['pageNum']);
    }
}
                                ...
                                <tr>
                                    ...
                                    <td width="874"><span class="table_data">分類：
                                        <select name="select1" id="select1" class="chosen-select">
                                            <?php $ReccityC->execute(); while($row_ReccityC = $ReccityC->fetch()): ?>
                                                <option value="<?php echo $row_ReccityC['c_id']?>" <?php if (!(strcmp($row_ReccityC['c_id'], $G_selected1))): ?>
                                                    <?= 'selected' ?>
                                                <?php endif ?>>
                            ...
                            <form action="shopC_process.php" method="post" name="form1" id="form1">
                                ...
                                <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1">
                                    ...
                                    <tr <?php if ($i%2==0): ?>bgcolor='#E4E4E4'<?php endif ?>>
                                        ...
                                        <td align="center" class="table_data">
                                            <a href="shopC_edit.php?c_id=<?php echo $row_RecshopC['c_id']; ?>">
                                        ...
                                        <td align="center" class="table_data"><a href="shopC_edit.php?c_id=<?php echo $row_RecshopC['c_id']; ?>"><img src="image/pencil.png" width="16" height="16" /></a></td>
                                        <td align="center" class="table_data"><a href="shopC_del.php?c_id=<?php echo $row_RecshopC['c_id']; ?>"><img src="image/cross.png" width="16" height="16" /></a></td>
                                    </tr>
...
<script type="text/javascript">
    function changeSort(pageNum, totalRows_RecshopC, now_c_id, change_num, selected1) {
        window.location.href = "shopC_list.php?selected1=" + selected1 + "&pageNum=" + pageNum + "&totalRows_RecshopC=" + totalRows_RecshopC + "&changeSort=1" + "&now_c_id=" + now_c_id + "&change_num=" + change_num;
    }
    ...
    $('#select1').change(function() {
        window.location.href = "shopC_list.php?changeSort=1&selected1="+$(this).val();
    });
</script>