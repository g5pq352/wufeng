...
$totalRowsC = $RecroomsC->rowCount();

$G_selected1 = '';
if (isset($_GET['selected1'])) {
    $_SESSION['selected_roomsC'] = $G_selected1 = $_GET['selected1'];
} else {
    $G_selected1 = $_SESSION['selected_roomsC'] = $row_RecroomsC['c_id'];
}

$query_Recrooms = "SELECT data_set.*, class_set.c_title as c_title FROM data_set LEFT JOIN class_set ON data_set.d_class2 =class_set.c_id WHERE d_class1 = 'rooms' AND d_class2='" . $G_selected1 . "' ORDER BY d_sort ASC, d_date DESC";
...
$totalRows = $all_Recrooms->rowCount();

$all_Recrooms = $conn->query($query_Recrooms);
$totalRows = $all_Recrooms->rowCount();
$totalPages_Recrooms = ceil($totalRows / $maxRows_Recrooms) - 1;
...
$queryString_Recrooms = sprintf("&totalRows=%d%s", $totalRows, $queryString_Recrooms);
...
if ($G_changeSort == 1 || $G_delchangeSort == 1) {
    ...
    $query_Recrooms = "SELECT data_set.*, class_set.c_title as c_title FROM data_set LEFT JOIN class_set ON data_set.d_class2 =class_set.c_id WHERE d_class1 = 'rooms' AND d_class2='" . $G_selected1 . "' ORDER BY d_sort ASC, d_date DESC";
    $_SESSION['selected_roomsC'] = $G_selected1;

    $Recrooms = $conn->query($query_Recrooms);
    $row_Recrooms = $Recrooms->fetch();

    do {
        if ($row_Recrooms['d_sort'] == 0) {} else if ($row_Recrooms['d_id'] == $_GET['now_d_id']) {
            ...
            $stat->bindParam(':d_id', $row_Recrooms['d_id'], PDO::PARAM_INT);
            $stat->execute();
    ...
    if ($G_changeSort == 1) {
        if (isset($_GET['now_d_id'])) {
            header("Location:rooms_list.php?selected1=" . $G_selected1 . "&pageNum=" . $_GET['pageNum'] . "&totalRows=" . $_GET['totalRows'] . "#" . $_GET['now_d_id']);
        } else {
            header("Location:rooms_list.php?selected1=" . $G_selected1 . "&pageNum=" . $_GET['pageNum'] . "&totalRows=" . $_GET['totalRows']);
        }
                                ...
                                <tr>
                                    ...
                                    <td width="874"><span class="table_data">分類：
                                        <select name="select1" id="select1" class="chosen-select">
                                            <?php do {?>
                                                <option value="<?php echo $row_RecroomsC['c_id']?>"<?php if (!(strcmp($row_RecroomsC['c_id'], $G_selected1))) {echo "selected=\"selected\"";} ?>><?php echo $row_RecroomsC['c_title']?><?php //echo $row_RecroomsC['c_id']?></option>
                                                <?php
                            ...
                            <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#E1E1E1" class="list_title_table">
                                <tr>
                                    <td width="739" align="right" class="page_display">
                                        <!-------顯示頁選擇與分頁設定開始---------->
                                        <?php displayPages($pageNum, $queryString_Recrooms, $totalPages_Recrooms, $totalRows, $currentPage); ?>
                                        <!-------顯示頁選擇與分頁設定結束---------->
                                    </td>
                            ...
                            <form action="rooms_process.php" method="post" name="form1" id="form1">
                                ...
                                <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1">
                                    ...
                                    do {
                                        $i++;
                                        $colname_RecImage = "-1";
                                        if (isset($row_Recrooms['d_id'])) {
                                          $colname_RecImage = $row_Recrooms['d_id'];
                                        }
                                    ...
                                    <tr <?php if ($i%2==0): ?>bgcolor='#E4E4E4'<?php endif ?>>
                                        <td align="center" class="table_data">
                                            <?php
                                            if(1){
                                                echo '<a href="rooms_edit.php?d_id='.$row_Recrooms['d_id'].'">'.$row_Recrooms['d_date'].'</a>';
                                            }else{
                                                echo $row_Recrooms['d_date'];
                                            }
                                            ?>
                                        </td>
                                        <td align="center" class="table_data">
                                            <select name="d_sort" id="d_sort" onchange="changeSort('<?php echo $pageNum; ?>','<?php echo $totalRows; ?>','<?php echo $row_Recrooms['d_id']; ?>',this.options[this.selectedIndex].value,<?= $G_selected1 ?>)">
                                                <option value="0" <?php if (!(strcmp(0, $row_Recrooms[ 'd_sort']))) {echo "selected";} ?>>至頂</option>
                                                <?php
                                                for($j=1;$j<=($totalRows);$j++) {
                                                    echo "<option value=\"".$j."\" ";
                                                    if (!(strcmp($j, $row_Recrooms['d_sort']))) {echo "selected=\"selected\"";}
                                                    echo ">".$j."</option>";
                                                }
                                                ?>
                                            </select>
...
<script type="text/javascript">
	...
	function changeSort(pageNum, totalRows, now_d_id, change_num, selected1) {
	    window.location.href = "rooms_list.php?selected1=" + selected1 + "&changeSort=1" + "&now_d_id=" + now_d_id + "&change_num=" + change_num + "&pageNum=" + pageNum + "&totalRows=" + totalRows;
	}

	$(document).ready(function() {
		$('#select1').change(function() {
			window.location.href = "rooms_list.php?&changeSort=1&selected1="+$(this).val();
		});
	});
</script>